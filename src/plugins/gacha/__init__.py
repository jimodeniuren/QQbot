...
shilian = on_command('shilian', aliases={'抽十连'})

@shilian.handle()
async def gacha_(bot: Bot, event: Event):
    G = Gacha()
    await bot.send(event,message = Message(G.gacha_10()))

xiaobaodi = on_command('xiaobaodi', aliases={'抽小保底'})

@xiaobaodi.handle()
async def gacha_(bot: Bot, event: Event):
    G = Gacha()
    await bot.send(event,message = Message(G.gacha_90(90)))



dabaodi = on_command('dabaodi', aliases={'抽大保底'})

@dabaodi.handle()
async def gacha_(bot: Bot, event: Event):
    G = Gacha()
    await bot.send(event,message = Message(G.gacha_90(180)))
